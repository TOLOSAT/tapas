...
#include "drivers/memories/qspimram.h"
#include "core/irq.h"
#include "fdir/fdir.h"
...
#define QSPIMRAM_READ_QUAD_IO_CMD        0xebu /**< Read quad I/O (1-4-4) */
#define QSPIMRAM_FAST_READ_CMD           0x0bu /**< Fast read (op depending on mode) */
#define QSPIMRAM_WRITE_CMD               0x02u /**< Write single I/O (1-1-1) */
#define QSPIMRAM_WRITE_QUAD_IO_CMD       0xd2u /**< Write quad I/O (1-4-4) */
#define QSPIMRAM_FAST_WRITE_CMD          0xdau /**< Fast write (op depending on mode) */
        ...
        if ((sector + length) <= (qspimram_inst->p_conf->size / QSPIMRAM_VIRTUAL_SECTOR_SIZE))
        {
            ...
            while ((nb_sector_written < length) && (return_value == RET_SUCCESSFUL))
            {
                ...
                if (return_value == RET_SUCCESSFUL)
                {
                    ...
                    qspi_command.InstructionMode     = qspimram_inst->p_conf->qpi ? QSPI_INSTRUCTION_4_LINES : QSPI_INSTRUCTION_1_LINE;
                    qspi_command.Instruction         = QSPIMRAM_FAST_WRITE_CMD;
                    qspi_command.AddressMode         = qspimram_inst->p_conf->qpi ? QSPI_ADDRESS_4_LINES : QSPI_ADDRESS_1_LINE;
    ...
    if ((qspimram_inst != NULL) && (data != NULL) && (length != 0u))
    {
        ...
        if ((sector + length) <= (qspimram_inst->p_conf->size / QSPIMRAM_VIRTUAL_SECTOR_SIZE))
        {
            ...
            while ((nb_sector_read < length) && (return_value == RET_SUCCESSFUL))
            {
                ...
                QSPI_CommandTypeDef qspi_command = { 0 };
                qspi_command.InstructionMode     = qspimram_inst->p_conf->qpi ? QSPI_INSTRUCTION_4_LINES : QSPI_INSTRUCTION_1_LINE;
                qspi_command.Instruction         = QSPIMRAM_FAST_READ_CMD;
                qspi_command.AddressMode         = qspimram_inst->p_conf->qpi ? QSPI_ADDRESS_4_LINES : QSPI_ADDRESS_1_LINE;
                qspi_command.AddressSize         = QSPI_ADDRESS_24_BITS;
                qspi_command.Address             = sector_base_addr;
                qspi_command.AlternateByteMode   = qspimram_inst->p_conf->qpi ? QSPI_ALTERNATE_BYTES_4_LINES : QSPI_ALTERNATE_BYTES_1_LINE;
