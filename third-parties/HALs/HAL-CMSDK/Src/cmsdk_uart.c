 ...
 * @retval      #HAL_ERROR if uart is a null pointer or baudrate is zero
 * @retval      #HAL_OK else
 */
HAL_StatusTypeDef cmsdk_UartInit(UART_HandleTypeDef *uart)
{
    ...
    if ((uart != NULL) && (uart->baud_rate != 0u))
    {
        // Set baud rate
 ...
 * @retval      #HAL_ERROR if uart or msg is a null pointer, if length is zero or state is error
...
HAL_StatusTypeDef cmsdk_UartTx(UART_HandleTypeDef *uart, uint8_t *msg, uint16_t length, uint32_t timeout)
{
    ...
    if ((uart != NULL) && (uart->gstate != HAL_UART_STATE_ERROR) && (msg != NULL) && (length != 0u))
    {
        ...
        if (uart->gstate == HAL_UART_STATE_READY)
        {
            ...
            // Transmit
            while ((uart->gstate != HAL_UART_STATE_ERROR) && (uart->tx_data_count < uart->tx_data_size) && (HAL_GetTick() < (tickstart + timeout)))
            {
...
HAL_StatusTypeDef cmsdk_UartRx(UART_HandleTypeDef *uart, uint8_t *msg, uint16_t length, uint32_t timeout)
{
    ...
    if ((uart != NULL) && (uart->gstate != HAL_UART_STATE_ERROR) && (msg != NULL) && (length != 0u))
    {
        ...
        if (uart->rxstate == HAL_UART_STATE_READY)
        {
            ...
            // Receive
            while ((uart->gstate != HAL_UART_STATE_ERROR) && (uart->rx_data_count < uart->rx_data_size) && (HAL_GetTick() < (tickstart + timeout))
                   && (uart->rxstate == HAL_UART_STATE_BUSY_RX))
            {
                // Got the new byte
                if ((uart->instance->STATE & CMSDK_UART_STATE_RXBF_Msk) == CMSDK_UART_STATE_RXBF_Msk)
                {
                    ...
                    if (uart->buffer_mode == HAL_UART_BUFFER_NORMAL)
                    {
                        uart->rx_data_count++;
                    }
                    else
                    {
                        uart->rx_data_count = (uart->rx_data_count + 1) % uart->rx_data_size;
                    }
                }
            }
 ...
 */
HAL_StatusTypeDef cmsdk_UartRxAbort(UART_HandleTypeDef *uart)
{
    HAL_StatusTypeDef status = HAL_OK;
